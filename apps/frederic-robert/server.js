...
async function proxyToSanity(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && response.status === 200) {
      ...
      console.log(
        '[sanity-proxy] MISS (no cache)',
        req.method,
        response.status,
        req.originalUrl.slice(0, 40),
      )
    }
...
app.use('/__purge', (req, res) => {
  ...
  proxyCache.clear()
  console.log(
    `[sanity-proxy] PURGED ${count} entries (token=${purgeToken ? 'yes' : 'no'})`,
  )
...
app.listen(port, () => {
  ...
  if (process.env.SANITY_PROXY_URL) {
    ...
    console.log(
      `[sanity-proxy] TTL=${CACHE_TTL / 1000}s | purge-token=${process.env.SANITY_PURGE_TOKEN ? 'set' : 'NOT SET'}`,
    )

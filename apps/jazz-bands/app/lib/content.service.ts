...
import type { Band, ContentService, Musician } from './types'
...
export class SanityContentService implements ContentService {
  private readonly BAND_QUERY = `
    *[_type == "band" && slug.current == $slug][0] {
      ...
      "members": bandMembers[] {
        _key,
        "musician": musician-> { _id, name, bio, instrument, "photo": images[0], gallery: images },
        bio,
        "images": images,
        instrument,
        sortOrder
  ...
  async getMusiciansByBandId(bandId: string): Promise<Musician[]> {
    try {
      const musicians = await sanityClient.fetch(this.MUSICIANS_BY_BAND_QUERY, {
        bandId,
      })
      return musicians || []
    } catch (error) {

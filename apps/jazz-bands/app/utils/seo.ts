 ...
 * and handling subdomain-based multi-tenancy SEO.
 */

import type { MetaFunction } from "react-router";

/**
 * Extract band slug from subdomain
 * Example: boheme.jazzbands.com -> "boheme"
 */
export function getBandFromSubdomain(host: string): string | null {
  const parts = host.split(".");
  
  // Handle: band.jazzbands.com or band.domain.tld
  if (parts.length >= 3 && parts[2].includes("jazzbands")) {
    return parts[0];
  }
  
  // Handle localhost development: boheme.localhost:5173
 ...
 */
export function getCanonicalUrl(request: Request, pathname = url.pathname): string {
  const url = new URL(request.url);
 ...
 */
export function mergeMeta(parentMeta: ReturnType<MetaFunction>, childMeta: ReturnType<MetaFunction>): ReturnType<MetaFunction> {

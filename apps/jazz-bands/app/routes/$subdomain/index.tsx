import { json, type LoaderFunctionArgs, Link, useLoaderData, useNavigation } from "react-router";
...
import { staggerContainerVariants, itemVariants, fadeUpVariants, buttonVariants } from "~/lib/animationVariants";
...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  if (!band) {
    throw new Response(`Band "${subdomain}" not found`, { status: 404 });
  }
  
  return json({ band, request });
}
...
export function meta({ loaderData }: { loaderData: Awaited<ReturnType<typeof loader>> | null }) {
  ...
  if (loaderData.band.heroImage) {
    const heroUrl = imageurl(loaderData.band.heroImage).width(1920).height(1080).url();
    meta.push({
      rel: "preload",
      as: "image",
      href: heroUrl,
...
{/* Hero Section with Parallax */}
           <section 
            ...
            >
             ...
             <div className="absolute inset-0 bg-gray-200 dark:bg-gray-700" aria-hidden="true" />
<motion.div
             ...
             >
               <h1 id="hero-title" className="text-5xl font-bold mb-4">{band.name}</h1>
...
{/* Musicians Section with Stagger Animation */}
           <section className="py-16 px-6 bg-gray-50" aria-labelledby="musicians-title">
             <div className="max-w-7xl mx-auto">
               <motion.h2
                 id="musicians-title"
...
{musician.photo && (
                       <img
                         src={imageurl(musician.photo).width(300).height(300).fit("crop").url()}
                         alt={musician.name}
                         loading="lazy"
                         decoding="async"
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-gray-600">{date.city}{date.region && `, ${date.region}`}</p>
                      </div>
                      {date.soldOut && (
